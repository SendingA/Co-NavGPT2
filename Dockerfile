...
LABEL org.opencontainers.image.title="FireNav"
LABEL org.opencontainers.image.description="Patched Habitat 0.3.3 multi-agent ObjectNav and FireWorld runtime"
LABEL org.opencontainers.image.revision="${VCS_REF}"

ENV LANG=C.UTF-8 \
    LC_ALL=C.UTF-8 \
    PATH=/opt/conda/envs/firenav/bin:/opt/conda/bin:${PATH} \
    CONDA_DEFAULT_ENV=firenav \
    FIRENAV_CONTAINER=1 \
    PYTHONUNBUFFERED=1 \
...
COPY docker/environment.yml /tmp/firenav-environment.yml

RUN conda env create --file /tmp/firenav-environment.yml \
    && conda clean --all --yes \
    && rm -f /tmp/firenav-environment.yml

COPY docker/requirements.lock.txt /tmp/firenav-requirements.lock.txt
...
RUN python -m pip install --no-cache-dir \
    ...
    && python -m pip install --no-cache-dir \
        --requirement /tmp/firenav-requirements.lock.txt \
    && python -m pip install --no-cache-dir --force-reinstall --no-deps \
        opencv-python==4.10.0.84 \
    && rm -f /tmp/firenav-requirements.lock.txt
...
COPY ref/habitat_lab_0.3.3_firenav.patch /tmp/habitat-firenav.patch

RUN git -C /opt/habitat-lab-src apply --check \
        /tmp/habitat-firenav.patch \
    && git -C /opt/habitat-lab-src apply /tmp/habitat-firenav.patch \
    && python -m pip install --no-cache-dir --no-deps \
        -e /opt/habitat-lab-src/habitat-lab \
        -e /opt/habitat-lab-src/habitat-baselines \
    && rm -f /tmp/habitat-firenav.patch
...
RUN chmod 0755 \
    ...
    && groupadd --gid 1000 firenav \
    && useradd --create-home --uid 1000 --gid 1000 \
        --shell /bin/bash firenav \
    && mkdir -p /workspace/outputs \
    && chown -R firenav:firenav /workspace/outputs
