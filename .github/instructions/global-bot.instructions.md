---
description: "Use when planning any non-trivial task, creating or moving files, updating TODOs, validating changes, or finishing a top-level user request. Defines global workflow policy: post-task follow-up, todo/todo.json tracking, file placement conventions (src/scripts/tests/docs/todo/assets/data/outputs/ref), mandatory validation after every change, completion checklist, and update discipline."
applyTo: "**"
---
...
Subagents are **strictly forbidden** from calling the ask-questions interface. The main agent must explicitly remind any subagent of this restriction in its prompt. Only the main agent may call ask-questions, and only when the current task is fully complete and ready for user review.
...
1. `src/` — Production source code and runtime modules.
2. `scripts/` — Automation scripts, one-off tooling, migration helpers.
3. `tests/` — Unit/integration/e2e tests mirroring `src/` structure.
4. `docs/` — User/developer documentation, architecture notes, ADRs.
5. `todo/` — Task tracking files (must include `todo.json`).
6. `assets/` — Static assets used by the project (images, fonts, templates).
7. `data/` — Input/reference datasets (`data/raw`, `data/processed`).
8. `outputs/` — Generated artifacts (reports, exported media, temporary generated files).
9. `ref/` — External references, experiments, or legacy material. Do not place core workflow code here.
10. Project root — Only for essential config and entry files (for example `package.json`, `pyproject.toml`, `README.md`, `.gitignore`).
