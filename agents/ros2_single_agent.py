...
class ROS_Agent(VLM_Agent):
    def __init__(self, args, agent_id, send_queue, receive_queue) -> None:
    ...
    def mapping(self, observations):
        ...
        current_point_sum = full_scene_pcd.voxel_down_sample(0.05)
        # current_point_sum = self.remove_full_points_cell(full_scene_pcd, self.camera_position)
        
        self.point_sum += current_point_sum
        
        self.update_map(full_scene_pcd, self.camera_position[1], 30.0 / 100.0 /2.0)

        target_score, target_edge_map, target_point_list = detect_frontier(self.explored_map, self.obstacle_map, self.current_grid_pose, threshold_point=20)
        ...
        if self.args.visualize or self.args.print_images:
            ...
            self.vis_map = self._visualize(self.obstacle_map, self.explored_map, target_edge_map, self.goal_map, self.goal_name)
    ...
    # def act(self):
        ...
        if len(target_point_list)> 0:
            goal_points = target_point_list[0]
        else:
            goal_points = np.random.rand(1, 2).squeeze()*(self.obstacle_map.shape[0] - 1)
    ...
    def _preprocess_depth(self, depth, min_d=0.5, max_d=4.0):
        ...
        depth[depth.shape[0]-40:, :] = 0.0
        depth[:, depth.shape[1]-40:] = 0.0
        depth[:40, :] = 0.0
        depth[:, :40] = 0.0
    ...
    def update_map(self, point_sum, camera_position_z, height_diff):
        ...
        z_max = camera_position_z + height_diff*2.5
