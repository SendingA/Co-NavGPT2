...
class EmbodiedTask:
    ...
    def step(self, action: Dict[str, Any], episode: Episode):
        ...
        # Co-NavGPTv3 multi-robot shortcut: a list/tuple of discrete Habitat
        # actions means "apply one classic navigation action per sim agent".
        # This preserves the v2 API shape: env.step([MOVE_FORWARD, TURN_LEFT]).
        if isinstance(action_name, (list, tuple)):
            if any(int(a) == 0 for a in action_name) and "stop" in self.actions:
                self.actions["stop"].step(task=self)
