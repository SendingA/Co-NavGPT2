...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def __init__(self, config: "SimulatorConfig") -> None:
        ...
        sim_sensors = []
        # Co-NavGPTv3 keeps the Habitat2-style observation contract:
        # reset/step return a list of per-agent observations with identical
        # sensor keys such as "rgb" and "depth".  Build the Lab sensor suite
        # from the default robot only, then attach the same sensor specs to
        # every Habitat-Sim agent in create_sim_config().
        default_agent_config = get_agent_config(self.habitat_config)
        for sensor_cfg in default_agent_config.sim_sensors.values():
    ...
    def reset(self) -> Observations:
        ...
        if self.config.enable_batch_renderer:
            ...
            return sim_obs
        elif len(self.habitat_config.agents_order) > 1:
            if not isinstance(sim_obs, list):
                sim_obs = [
                    self.get_sensor_observations(agent_ids=i)
                    for i in range(len(self.habitat_config.agents_order))
                ]
            return [
                self._sensor_suite.get_observations(agent_obs)
                for agent_obs in sim_obs
            ]
        else:
            return self._sensor_suite.get_observations(sim_obs)

    def step(
        self, action: Optional[Union[str, np.ndarray, int, List[int], tuple, Dict[int, int]]]
    ) -> Observations:
        if action is None:
    ...
    def set_agent_state(
        ...
        new_state.position = mn.Vector3(position)
