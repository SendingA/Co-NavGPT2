"""Visible Habitat3 robot models for classic ObjectNav agents.

The ObjectNav agents remain Habitat-Sim navigation agents.  This helper loads
Habitat3 articulated robot assets as kinematic visual models and synchronizes
them to the navigation agents after reset/step.
"""
...
ROBOT_PROFILES: Dict[str, RobotProfile] = {
    "fetch": RobotProfile(
        FetchRobot,
        "Co-NavGPTv3/data/robots/hab_fetch/robots/hab_fetch.urdf",
        "Fetch mobile manipulator.",
    ),
    "fetch_no_wheels": RobotProfile(
        FetchRobotNoWheels,
        "Co-NavGPTv3/data/robots/hab_fetch/robots/fetch_no_base.urdf",
        "Fetch upper body without wheel links.",
    ),
    "fetch_suction": RobotProfile(
        FetchSuctionRobot,
        "Co-NavGPTv3/data/robots/hab_fetch/robots/hab_suction.urdf",
        "Fetch suction-gripper variant; requires the suction URDF asset.",
    ),
    "spot": RobotProfile(
        SpotRobot,
        "Co-NavGPTv3/data/robots/hab_spot_arm/urdf/hab_spot_arm.urdf",
        "Boston Dynamics Spot arm model; requires Habitat3 Spot assets.",
    ),
    "stretch": RobotProfile(
        StretchRobot,
        "Co-NavGPTv3/data/robots/hab_stretch/urdf/hab_stretch.urdf",
        "Hello Robot Stretch model; requires Habitat3 Stretch assets.",
    ),
}
...
class RobotModelManager:
    ...
    def __init__(
        ...
        self.profile_names = self._as_list(profiles)
    ...
    def _ensure_urdf_exists(profile_name: str, urdf_path: str) -> None:
        ...
        raise FileNotFoundError(
            ...
            "Place Habitat robot assets under Co-NavGPTv3/data/robots, or set "
            "conav.robot_model_urdfs to an existing URDF path."
        )
