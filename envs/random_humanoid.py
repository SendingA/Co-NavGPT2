...
class RandomHumanoidWalker:
    ...
    def humans(self) -> List[KinematicHumanoid]:
        return [state.humanoid for state in self._states if state.active]

    def reset(
        self,
        spawn_positions: Optional[Sequence[Sequence[float]]] = None,
        *,
        static: bool = False,
    ) -> None:
        """Place random walkers or bind fixed humanoids to dataset goals."""
        if static and spawn_positions is None:
            raise ValueError("static humanoids require explicit spawn positions")

        desired_count = (
            len(spawn_positions)
            if spawn_positions is not None
            else self.num_humans
        )

        while len(self._states) < desired_count:
            self._states.append(self._make_state(len(self._states)))

        for index, state in enumerate(self._states):
            if (
                state.humanoid.sim_obj is None
                or not state.humanoid.sim_obj.is_alive
            ):
                state.humanoid.reconfigure()
            state.active = index < desired_count
            state.static = bool(static and state.active)
            if not state.active:
                self._deactivate_humanoid(state)
            elif spawn_positions is not None:
                self._place_humanoid_at(state, spawn_positions[index])
            else:
    ...
    def step(self) -> None:
        ...
        for state in self._states:
            if not state.active or state.static:
                continue
    ...
    def _make_state(self, human_index: int) -> _WalkerState:
        ...
        ctrl_freq = 1.0 / self.motion_dt if self.motion_dt > 0 else float(
            getattr(
                getattr(self.sim, "habitat_config", None),
                "ctrl_freq",
                120.0,
            )
        )
    ...
    def _place_humanoid(self, state: _WalkerState) -> None:
        ...
        self._set_humanoid_pose(state, human_pos, yaw)
        state.target = self._sample_reachable_target(state.humanoid.base_pos)

    def _place_humanoid_at(
        self, state: _WalkerState, position: Sequence[float]
    ) -> None:
        human_pos = mn.Vector3(np.asarray(position, dtype=np.float32))
        self._set_humanoid_pose(state, human_pos, 0.0)
        state.target = None

    @staticmethod
    def _set_humanoid_pose(
        state: _WalkerState, human_pos: mn.Vector3, yaw: float
    ) -> None:
    ...
    @staticmethod
    def _deactivate_humanoid(state: _WalkerState) -> None:
        state.target = None
        state.humanoid.base_pos = mn.Vector3(0.0, -1000.0, 0.0)
        state.humanoid.update()
